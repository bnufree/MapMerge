...
void            S572Senc(ChartTableEntry* e)
{
    if(!e) return;
    e->createSencFile();
}
...
void ChartFrameWork::slotUpdateChartDatabase(ArrayOfCDI &DirArray, bool b_force, const QString &filename)
{
    ...
    //开始多线程生成SEC文件
    {
        ...
        QObject::connect(&futureWatcher, SIGNAL(progressRangeChanged(int,int)), this, SIGNAL(signalSendProcessRange(int,int)));
        QObject::connect(&futureWatcher, SIGNAL(progressValueChanged(int)), this, SIGNAL(signalSendProcessVal(int)));
...
void ChartFrameWork::SetGroupIndex( int index, bool autoSwitch )
{
    ...
    int dbi_now = GetQuiltReferenceChartIndex();
...
{
    return  (int) m_pQuilt->GetRefNativeScale();
}
...
{
    ChartBase* target_chart = m_pQuilt->GetOverlayChartAtPix( mViewPoint, QPoint( x, y ) );
    return target_chart;
}
