...
void ZCHXRadarDataChange::appendRadarRect(const ZCHX_RadarRect_Param &param)
{
    ...
    connect(this, SIGNAL(signalRectFilterSecsChange(int)), thread, SLOT(setFilterTime(int)));
    mThreadList.append(thread);
    thread->start();
...
void ZCHXRadarDataChange::setRectFilterSecs(int secs)
{
    mRectFilterSecs = secs;
    emit signalRectFilterSecsChange(secs);
}
